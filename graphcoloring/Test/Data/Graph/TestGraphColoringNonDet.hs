...
module Test.Data.Graph.TestGraphColoringNonDet
  (tests)
  where
...
graphTest :: ByteString -> String -> [Color] -> IO ()
graphTest gFile name expected =
  let g = readEdgeArray gFile
      g' = invertEdgeArray g
        in U.toList <$> graphColoringNonDeterministic g g' >>= \y ->
           assertEqual name expected y
