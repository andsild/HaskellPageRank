...
--TODO: I think the way I'm suppose to use unsafeIO is to return the new value in this function, and then use unsafe inside for reading, etc. That way the value will be forced. But will I save anything?
--FIXME: unsafePerformIO throws away the state token and replaces it with RealWorld#.
{-# INLINE vertexLoop #-}
vertexLoop ::
  MU.IOVector PRType ->
  Int ->
...
pageRank :: (NeighborGraph g) => g -> g -> PRType -> PRType -> PRType -> Word -> IO (U.Vector PRType)
